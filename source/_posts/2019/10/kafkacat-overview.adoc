...
`-P` -- produce mode (also available, `-C` -- consume, `-L` -- metadata, `-Q` -- query)
...
...
Key (1 bytes): 6
Value (90 bytes): {"order_id":6,"order_ts":1534773042276,"total_amount":31.00,"customer_name":"Emma Watson"}
